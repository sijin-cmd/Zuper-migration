...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (module === 'jobs' && sub === 'checklists') {
      ...
      if (!categoryUid || !statusUid) return respond(200, { data: [] });
    ...
    if (module === 'customfields') {
      ...
      if (!CF_MODULES.includes(modName)) return respond(400, { error: 'Unknown module: ' + sub });
      const r = await zuperFetch(
        session.srcBase, session.srcKey,
      ...
      return respond(200, { groups: [...groupsMap.values()] });
    }

    return respond(400, { error: `Unknown: ${module}/${sub}` });

  } catch (err) {
    return respond(500, { error: err.message || 'Request failed', status: err.status || null });
  }
};
